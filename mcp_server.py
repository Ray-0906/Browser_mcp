...
def _require_app_context(ctx: Context[ServerSession, AppContext]) -> AppContext:
...
async def app_lifespan(_: FastMCP[AppContext]) -> AsyncIterator[AppContext]:
    ...
    session_manager = SessionManager()
    try:
        yield AppContext(browser_service=browser_service, session_manager=session_manager)
    finally:
...
async def create_session(
    ...
    *,
    ...
    app_ctx = _require_app_context(ctx)
    try:
...
async def active_sessions_resource(ctx: Context[ServerSession, AppContext]) -> Dict[str, Any]:
    app_ctx = _require_app_context(ctx)
